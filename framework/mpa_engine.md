...
## DEFORMATION GENERATORS — the character deformation chart (conditional)

**The character deformation chart.** A character-bearing drift admits a finite, *closed* set of deformations; tabulating each generator's forced response is a universal **coordinate chart for substrate character** (the map, not the substrates). Two tiers.

* **Linear core** — the deformation space of $M=-\gamma I+g\,A_{\text{CYC}}$ is $\mathfrak{gl}(3,\mathbb{R})$, **Cartan-decomposed**, **exhaustive** ($\dim=1{+}3{+}5=9$) and **closed** ($[\mathfrak{k},\mathfrak{k}]\subseteq\mathfrak{k}$, $[\mathfrak{k},\mathfrak{p}]\subseteq\mathfrak{p}$, $[\mathfrak{p},\mathfrak{p}]\subseteq\mathfrak{k}$; $\mathfrak{k}{=}\mathfrak{so}(3)$, $\mathfrak{p}{=}\mathrm{Sym}_0$; commutator residual $3\!\cdot\!10^{-16}$) — a closure (a law), not a catalogue:
  * **Damping** ($\mathbb{R}I$, center, 1) — uniform relaxation-rate shift $\to$ trivial $\mathrm{Re}$-shift.
  * **Chirality** ($\mathfrak{so}(3)$, 3) — rotation: magnitude · sign · orientation (axis); $A_{\text{CYC}}=L_x{+}L_y{+}L_z$, axis $(1,1,1)$, carries the chimeric sign $\to$ sign-flip at the achiral point (pitchfork centre); EP under splitting.
  * **Splitting** ($\mathrm{Sym}_0$, 5) — anisotropic detuning $\to$ non-Hermitian **EP** $\omega^2=\omega_0^2-(\delta/2)^2$, EP at $\delta=2\omega_0$.
* **Closed extensions** (each closes on a named structure beyond $\mathfrak{gl}(3)$): **Noise** $=$ diffusion tensor $\in\mathrm{Sym}^+$ (continuous Lyapunov) $\to$ stationary covariance/current; **Nonlinearity** $=$ gain/saturation normal forms $\to$ pitchfork/Hopf; **Drive-periodicity** $=$ Floquet/monodromy $\to$ parametric resonance $\to$ Adler circle map; **Composition** $=$ Schur on coupled triads $\to$ again $(\mathfrak{gl}(3)$ drift, $\mathrm{Sym}^+$ noise$)$, the two-mode-kernel RG type-identity (ties `COMPRESSION`).
* **Not primitives — tools / landmarks.** The $\sim39°$ symmetric-cone chirality-reversal node and the non-uniform-squeeze secondary tongues are model-specific *features*, not generators (stay frontier, K2). They are the chart's **falsifiability handles**: model-specific landmarks by which one side of a character's layer recovers the other. **Use:** each forced response is a checkable prediction; even a *gross* (low-resolution) chart has teeth — it **rules out** substrate configurations (atomic / molecular / …) whose character cannot compose from these generators, or which lack a predicted landmark where the chart requires one.
...
* **FALSIFY**: a triad deformation that is not a composition of the chart's generators (basis not closed); a non-model-specific response that misses its canonical closed form; the protecting symmetry only hand-suppliable (neither derived nor honestly conditioned $=$ fitted).

*Coverage instanced 2026-05-30 on a real emergent substrate — rock-paper-scissors (`mpa-conform/scripts/rps_character.py`): RPS's emergent Jacobian composes from the chart — its chirality channel **is** the emergent $A_{\text{CYC}}$ (residual 0, forced onto the $(1,1,1)$ axis, not hand-drawn), the EP normal form holds on the emergent plane ($R^2{=}1.0000$), and the conditional lift closes on the C3-coupled meta-arena (b1 3→4; $\omega_{\text{meta}}\propto\kappa^2$ vs split $\propto\kappa$). **Promoted**; receipts §deformation-generators.*
...
* **CONSTRUCTION**: the two frames are **one dissipation, two readings** — not two independent observables. The two-frame iff-chain
...
ties the FDR reading to the topological claim in both directions:
...
* Each claim = a predicted measurement on a named substrate/class with a formal refutation condition.
