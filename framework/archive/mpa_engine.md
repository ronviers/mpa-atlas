...
* **R** $:\mathcal{M}\to\mathcal{M}$ — sever to bath, cost $W_R/\kappa\ge\ln2\cdot H(A\mid\text{rest})$ (recovers $\ln2$ floor at $\kappa=k_BT\,\xi_{sub}$). **Limit $\to\neg$.**
* $\top,\bot \to 1,0$.
...
* **Amplitude bit**: $D\to\infty$ endpoint of chit; occupancy (which-state); emergently discrete; erased by $R$ at floor $\ln2$; held vs thermal leak; reversibly flippable at no fundamental cost.
* **Topological bit**: chimeric sign of a frustrated triad; wiring (which-handedness); intrinsically discrete $\forall$ finite $D$; gauge-irremovable; $\tau_{obs}$- and amplitude-invariant; **free to hold** (no barrier leak) but **not reversibly flippable** — chirality change is forced through the balanced (sign-erased) state, $\ge\ln2$ per protected sign. $\Rightarrow$ **cost moves from maintenance to modification.**
* Forced-erasure floor = cycle-space dimension of the frustrated subgraph.
* **FALSIFY**: protected sign requires a per-time maintenance cost scaling with held duration (amplitude bit in disguise); OR chirality flips along a continuous path below $\ln2$.
...
**Two conjugate frames** (verdict-agreement = claim, disagreement = falsifier):
* external-probe: amplitude $\times$ field $\to X$ (needs a probe).
* self-probe: $J\times\mathcal{A}\to$ tightness $T=\langle\sigma\rangle\,\mathrm{Var}(J)/(2\langle J\rangle^2)$; core $\mathrm{SNR}_J=\langle J\rangle^2/\mathrm{Var}(J)\le\langle\sigma\rangle/2$; defined iff a current exists. Bridge: $\int$FDR-violation $=\langle\sigma\rangle=J\!\cdot\!\mathcal{A}$.

## THERMO ↔ INFO DUAL LEDGER

| axis | thermodynamic | informational |
|---|---|---|
| per-event | chit$=\ln(G_0/L)$ | bit$=\ln2$ |
...
* **Topological bit**: protected sign requires a per-time maintenance cost scaling with held duration (amplitude bit in disguise), OR chirality flips along a continuous path below $\ln2$.
...
* **Two frames**: external-probe and self-probe disagree on regime verdict where both are computable.
