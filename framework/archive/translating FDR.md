...
- **Does the translated apparatus integrate into the character framework?** Yes — see [`mpav1_compressed.md`](mpav1_compressed.md) §4 (the MPA Response Apparatus / FDR signatures) for the current FDR-derived material: correlation function, response function, X-ratio, parametric plot, aging diagonal, effective temperature as direct transfers; active modulation, substrate-equivalent temperature, multi-window reading, and NESS-by-default reframing as required extensions. (Section number was §7 in earlier versions; the laser-physics restructure of 2026-05-10 moved gFDR forward to §4. This methodology document predates the restructure and may need its own refresh.)
...
Earlier drafts presented the chit as a candidate unit with the right Shannon shape. The Harada–Sasa relation connects FDR-violation to entropy production rate, and the chit-rate $\ln(G_0/L)$ corresponds to dimensionless entropy production per holding-event. This converted the chit from "candidate with the right shape" to "unit with a thermodynamic derivation." Landed in [`mpav1_compressed.md`](mpav1_compressed.md) §"The chit unit".
...
For sustained NESS holdings, measuring spontaneous fluctuation spectrum + response to small probes yields the entropy production rate $\langle\sigma\rangle$ via integrated FDR-violation. Spontaneous observation suffices for rate measurement; no large-load imposition required. Landed in [`mpav1_compressed.md`](mpav1_compressed.md) §"FDR signatures".
*Subsequent deepening (2026-05-21) — two conjugate frames [STAGED, not promoted]:* Pressing the Harada–Sasa protocol against irreversible-thermodynamics force–flux pairing (step-5 machinery: Schnakenberg cycle affinity, the thermodynamic uncertainty relation) surfaced that the fluctuation-response relation has **two conjugate frames**, not one. The standard *external frame* reads the (amplitude × external field) pair → violation factor $X$ — it needs an external probe and is substrate-conditional. The *self-probe frame* reads the (current × intrinsic affinity) pair → violation factor $T$ (the TUR-tightness), **dimensionless by construction** and **defined iff a current exists** (broken detailed balance / $k_{\text{frust}}$-bearing). Harada–Sasa is the bridge: the externally-measured integrated violation equals the intrinsic $\langle\sigma\rangle = J\!\cdot\!A$, so the two frames read one dissipation from two directions. This makes Implication 2's "spontaneous observation suffices" precise — the self-probe frame *is* the probe-free reading. It is a methodology-process instance of this document's thesis (working out the translation updates the framework): the second frame was invisible until FDR was pressed against the force–flux machinery. Staged in [`mpav1_receipts.md`](mpav1_receipts.md) §FDR/§16 (STAGED CANDIDATE), gated on a real-substrate $T$-vs-$X$ agreement before promotion to `mpav1_compressed`; ledger in [`H:/mpa-central/FALSIFICATION.md`](../../mpa-central/FALSIFICATION.md) §TWO-FRAME gFDR.

**Implication 3: Active-probe vs passive-probe distinction. [LANDED]**
The framework's active-probe extension acquired precise mathematical content via the damping/resonance adoption. Probes within $\gamma_{RO}$ of the relaxation-oscillation frequency $\omega_{RO}$ are *active* (Q-amplified, engage maintenance feedback through the relaxation-oscillation loop); off-resonance probes are *passive* baseline. Boundary linewidth = $\gamma_{RO}$. Channel-capacity bound (Shannon–Hartley) is the informational dual of TUR. Landed in [`mpav1_compressed.md`](mpav1_compressed.md) §"Stability and attractor structure" + §"Thermodynamic and informational accounting". Receipts §13 + §17.

**Implication 4: Window-dependence becomes first-class. [LANDED via inheritance]**
Inherited from the STRUCTURAL reading's `§Scale-relativity` discipline; the CHARACTER reading respects the same kernel-width-dependence throughout. The §"FDR signatures" per-regime invariants are window-dependent, with $\alpha_s$ and $P_s$ as the load-bearing cross-substrate observables.

**Implication 5: Earlier §7 framing refactored. [HISTORICAL]**
The 2026-05-09 refactor replaced mechanical-engineering force-deformation analogues with FDR-derived content. The 2026-05-10 laser-physics restructure adopted the STRUCTURAL reading's section structure. The 2026-05-10 unabridged-rename further restructured: dense narrative now lives in [`mpav1_unabridged.md`](mpav1_unabridged.md), claim-only operational source of truth in [`mpav1_compressed.md`](mpav1_compressed.md). The mechanical-engineering vocabulary is no longer in the corpus.
