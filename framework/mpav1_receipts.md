# MPA receipts (unified)

**Companion to:** [`mpav1_compressed.md`](mpav1_compressed.md) (the operational source of truth) and
[`mpav1_unabridged.md`](mpav1_unabridged.md) (public-facing prose version, rebuilt periodically).
**Status:** Live. Append-only during prove-as-you-go work; the file that makes unabridged
reconstruction tractable. Originally built by folding the split `v9_receipts` + `cdv1_receipts`
into one file under the unified section structure (archived at
[`archive/2026-05-23_unification/`](archive/2026-05-23_unification/)); now the sole canonical
receipts file.

If receipts and compressed disagree, treat as bug; the compressed is canonical where all three differ.

---

## Workflow

When a session proves, derives, or composes a result that becomes a line in `mpav1_compressed.md`:

1. Write the compressed line as usual.
2. **In the same session**, append an entry here under the matching section (concept name for the
   spine/structural block; the existing `§N — Concept` header for the register/character block).
3. Tag with a type (below).
4. Don't leave the session without at minimum a type tag and a one-line summary.

**No retrofitting.** Lines whose proof tree is gone get `unrecovered` and a brief note. An honest
marker beats a plausible-sounding fabrication.
...
- `recovered-from-chat` — derivation was lost from the docs but recovered by *transcription* from a prior session transcript (not rederived). Name the source session/transcript; flag if bounds-only vs full.
- `open` — known open question carried forward from compressed.
- `empirical` — empirical claim. Point to dataset + analysis.

## Keying (two conventions, both preserved verbatim)

The two source projections used different — both valid — keying conventions; the merge preserves
each rather than forcing one scheme (a re-key would be lossy transcription, and the `§N` track the
unabridged numbering that Phase 5 rebuilds anyway):

- **Spine & structural-wing entries (v9 projection)** are **concept-keyed** and resolve by
  **section-name match** to the compressed (e.g. receipts `§Capacity` ↔ compressed `§Capacity`).
  v9_compressed carried no inline pointers; that convention is inherited here. Where a unified
  section was *renamed* in the merge, the §Section-map below carries the routing.
- **Register & character-wing entries (cdv1 projection)** keep their `### §N — Concept name`
  headers. The `§N` is what the inline `Receipts §N` pointers in the compressed resolve against;
  **the concept name is the stable anchor** (the `§N` tracks the unabridged numbering; the restructure
  that rebuilt it is archived at [`archive/2026-05-23_unification/identities_restructure_spec.md`](archive/2026-05-23_unification/identities_restructure_spec.md)). Future
  restructures preserve concept names, not numbers.

## §Section-map — unified compressed section → contributing receipts entries

Routing index for the merged structure. `[v9]` = concept-keyed entry in the spine/structural block;
`[cdv1 §N]` = numbered entry in the register/character block. An entry may serve more than one
section (co-housed); that is honest, not duplication.

| Unified compressed section | Contributing receipts entries |
|---|---|
| **SPINE** | |
| Setting & primitives | `[v9]` §Three typed objects · `[cdv1 §1]` bridge |
| The deformation & its two faces | `[v9]` §Boolean section · `[v9]` §Operators (R bound, referenced) · `[cdv1 §2]` chit/$G_0$ deformation coord · `[cdv1 §10]` $k_{\text{frust}}$ derived · `[cdv1 §Central commitment]` · `[cdv1 §Two bits / topological-Landauer]` · `[cdv1 §Homochirality]` |
| Typed objects & the bridge | `[v9]` §Three typed objects · `[cdv1 §1]` bridge |
| Operators | `[v9]` §Operators · `[cdv1 §7]` Thm 9 via adiabaticity |
| Scale-relativity | `[v9]` §Trail-class metric |
| Composite catalogue | `[cdv1 §9]` two-mode pairs · `[cdv1 §16]` Schnakenberg per row |
| Capacity | `[v9]` §Capacity · `[cdv1 §5]` $\eta$ Erlang-B · `[cdv1 §22]` load-handling · `[cdv1 §5/§22]` hard-vs-soft walls |
| FDR signatures | `[v9]` §Surface-code identification · `[v9]` §Substrate-conditional reading (sign caveat) · `[cdv1 §gFDR]` two-step · `[cdv1 §gFDR/§16]` two-frame · `[cdv1 §4]` surface-code falsifier |
| Compression / heat-tax | `[v9]` §Compression Axiom · `[v9]` §RG closure · `[v9]` §Trail-class metric · `[cdv1 §6]` heat-tax relation · `[cdv1 §6.5]` meta-ledger flow · `[cdv1 §17]` info content · `[cdv1 §18]` SOC/branching |
| k_frust drain | `[cdv1 §10]` derived · `[cdv1 §Topological-drain / §8]` PROMOTED · `[cdv1 §13/§16/§Topological-drain]` mpa-LEGAL audit · `[cdv1 §13]` homotopy obstruction · `[cdv1 §16]` Schnakenberg affinity |
| Asymptotic closure | `[v9]` §Asymptotic closure · `[cdv1 §9]` $\beta_{\text{mem}}=1$ Markovian limit |
| Substrate-conditional reading rules | `[v9]` §Substrate-conditional reading |
| **STRUCTURAL WING** | |
| Deformation calculus | `[v9]` §Deformation calculus (Thm 9) · `[cdv1 §7]` Thm 9 adiabaticity (alt path) |
| Extension axes / Falloff profile | definitional — no receipts entry |
| **CHARACTER WING** | |
| The chit unit | `[cdv1 §2]` chit definition |
| Fraying sequence | `[cdv1 §14]` attractor classification · `[cdv1 §18]` criticality |
| Universal two-mode kernel | `[cdv1 §9]` bifurcation + closed form · `[cdv1 §15]` MSF/sync · `[cdv1 §21]` non-reciprocal portrait · `[cdv1 §22]` falsifier (reframed) |
| Relaxation-oscillation register | `[cdv1 §13]` $\omega_{RO}$ class-B laser · `[cdv1 §14]` Wall-forces-NRT delay-Hopf |
| Thermodynamic & informational accounting | `[cdv1 §16]` TUR/Schnakenberg · `[cdv1 §17]` bit/chit dual ledger · `[cdv1 §17/§20]` $I_{\text{pred}}$ · `[cdv1 §15]` cryptic order / optimal encoding |
| Adoption catalogue | `[cdv1 §11–§22]` discipline closures (queueing §22, control §20, active matter §21, SOC §18, pattern formation §19, sync §15, …) |
| Five leading-order posits | `[cdv1 §9]` $\beta_{\text{mem}}$ · `[cdv1 §14]` $\mu=e^{\text{chit}}$ (Galton–Watson) · `[cdv1 §20]` $w_i$ auto-tuning · `[cdv1 §22/§17]` $u_n=\epsilon_n$ · `[cdv1 §15]` $\chi=\Delta_n$ |
| **OPEN ITEMS** | falsifier-formalization + structural-conjecture blocks (cdv1 §§4, 5/22, 14, 15, 16, 17/20, 18, 19, 21, 22) |

---

## Spine & structural-wing entries (v9 projection — concept-keyed; resolve by section-name match)
...
### §Boolean section — restriction $\Sigma \to \mathcal{M}_2 \cong (\mathbb{B},\land,\lor,\oplus,\neg)$ IS the finite-$D$ deformation of the $\{\oplus,\land,1\}$ (Reed-Muller / ANF) ring
**Type:** composition.
**Cite:** Reed–Muller / algebraic normal form over GF(2) — every Boolean function has a unique $\{\oplus,\land,1\}$ representation, with $\neg x = x \oplus 1$ (standard). $\mathcal{M}_2 \cong (\mathbb{B},\land,\lor,\oplus,\neg)$ is the existing compressed identification.
**Bespoke step:** Type-identity. The compressed restriction is *already* the ANF ring under $\oplus\leftrightarrow K$ (parity: $c$ iff modes differ), $\land\leftrightarrow C$, $1\leftrightarrow\top$. The ANF closure identity $\neg x = x\oplus 1$ then holds as $K(\cdot,\top)=\neg$ — pure GF(2), no new machinery. So "integrate Reed-Muller" is *recognition*, not a spine refactor; v9's Boolean section IS the deformed ring, and MPA (finite $D$, §Boolean section line 13) is its deformation away from the $D\to\infty$ Boolean limit.
**Two rejected legs (closure test resolves them):**
- $1\leftrightarrow G_0$ — **category error.** $K:\mathcal{M}^2\to\mathcal{M}_2$ takes two modes; $G_0\in\mathbb{R}_+$ is the drive/budget ($\text{chit}=\ln(G_0/L)$), not an $\mathcal{M}_2$ element, so $K(\cdot,G_0)$ is type-ill-formed. The ANF $1$ is $\top$. $G_0$'s real role is the *deformation coordinate* $\text{chit}=\ln(G_0/L)$ — where you sit in the $c\to s\to r$ melt (cdv1) — not the logical constant.
- $R\leftrightarrow\neg$ — one-way ($c\to r$) shadow, **not** a dynamical involution. $R$ severs to bath irreversibly ($R\circ R = R \ne \mathrm{id}$), matching $\neg$ only on the $c$ branch. $R$'s irreversibility *is* the finite-$D$ deformation of the ring's involution axiom (the involution $\neg=\oplus 1$ is carried by $K(\cdot,\top)$, not $R$). Consistent with the one-way quotient already noted at §Operators ($R$ entry).
**Receipt:** `mpa-central/library/rm_closure_test.py` (closure audit at the Boolean endpoint using exact §Operators definitions; PNG `output/diagnostics/rm_closure_test.png`). Cross-ref: cdv1_receipts §2 (the $\text{chit}=\ln(G_0/L)$ deformation coordinate $G_0$ enters as) and §13/§14 ($c\to r$ as terminal/irreversible — $R$'s deformed-involution role).
**Bearing on $k_{\text{frust}}$:** $k_{\text{frust}}=$ UNSAT-of-$K$ is well defined ($K\leftrightarrow\oplus$ is solid); its §846 promotion stays separately gated on a real topologically-forced instance. The closure test does not bear on that gate.
**Two deformation faces (added 2026-05-22).** The deformation off Boolean carries two coordinates: **amplitude** (chit/$D$ — drives the $c\to s\to r$ migration) and **sign-topological** (Harary balance). Boolean = the balanced/gaugeable ring (Harary 1953: balanced iff every cycle has even negative parity ⇒ gauge-equivalent to unsigned ⇒ real spectrum, no protected current). The sign-face deformation is Harary-*imbalance*; its minimal carrier is the Harary triad $\triangle_H$ and its order parameter the affinity $\mathcal{A}=\oint v/D$ (cdv1 §Central commitment). The chimeric sign of $\triangle_H$ IS the deformation coordinate away from Harary-balance — the ANF deformation made dynamical. Topological, not amplitude ⇒ $k_{\text{frust}}$ does not migrate under the $\tau_{obs}$ sweep while $c/s/r$ does (§Scale-relativity) — the bridge closes the framework end-to-end (Boolean-gaugeable ↔ MPA-deformed, one structure at two depths). No new object: the ring (Reed-Muller/ANF), balance (Harary), cycle (May–Leonard), affinity (Schnakenberg) are all imported; MPA only reads them under sustained dissipation.
...
### §Deformation calculus — Theorems 6, 7, 9
**Type:** recovered-from-chat (bounds-only).
**Provenance:** the three defect statements were recovered by transcription (not rederivation) from the 2026-05-02 "MPD foundation refactor" session (transcript `22075bf3-d22f-4e63-8937-7d399d69c88b`). This closes the prior `unrecovered` marker at the **leading-bound** level. The explicit $1/D$ operator *series* beyond leading order (defects as full trail-vector functionals) is still owed — see compressed §Owed work; recovering the bounds does not recover the series.

**Theorem 6 (associator).** $\alpha_C(A,B,C) = C(C(A,B),C) \ominus C(A,C(B,C))$, with
$$\lVert\alpha_C\rVert \lesssim \frac{\kappa}{\Phi^*}\big(\lvert\gamma_{AB}\rvert+\lvert\gamma_{BC}\rvert+\lvert\gamma_{AC}\rvert\big) \xrightarrow{\Phi^*\to\infty} 0.$$

**Theorem 7 (distributivity defect).** $\delta_{\text{dist}}(A,B,C) = C(A,S(B,C)) \ominus S(C(A,B),C(A,C))$, with
$$\lVert\delta_{\text{dist}}\rVert \lesssim \frac{\kappa}{\Phi^*}\big[\max(0,\gamma_{BC}) - \max(0,\gamma_{AB},\gamma_{AC})\big]^+ \xrightarrow{\Phi^*\to\infty} 0.$$

**Theorem 9 (Boolean deviation).** $\Delta_C(A,B) = \sigma(C(A,B)) \oplus (\sigma(A)\land\sigma(B))$, equal to $1$ iff $\gamma_{AB} > 0$ and $\Phi^* < \kappa\,\gamma_{AB}$ (equivalently $D < \gamma_{AB}$, since $D=\Phi^*/\kappa$).

These match the compressed §Deformation calculus table verbatim ($\kappa/\Phi^*=1/D$; $\Phi^*<\kappa\gamma_{AB}\Leftrightarrow D<\gamma_{AB}$); the recovery supplies the operator-level defect *definitions* and the derivation chain ordering (associator bound → distributivity defect → Boolean deviation) the compressed table abbreviates.
**Note:** Character §7 recovers Theorem 9 as the discrete-time limit of the adiabaticity bound — an *alternative* justification path, not a reconstruction of this original; both now stand.
**Sharpening status (Test B, 2026-05-26).** The bound→equality sharpening (the owed $1/D$ *series*) was explored via the outbound model channel; results are **conditional-on-closure and currently fitted-not-forced**, not promoted. Summary: Thm 6 looks like a genuine $D^{-1}$ series ($p=1$, trilinear coefficient in shears × trail-overlaps × merge-derivative) conditional on smooth-merge closures; Thm 9 is a **critical boundary-layer crossover, not a $1/D$ series** (character-series candidate via the FD-plot / KWW five-vector, but fitted with ≥5 free params and validated only on the synthetic `two_temp_ou` control — not earned evidence); Thm 7 + the single-bracket question (Test C) hinge on whether $S$'s $\max(0,\gamma)$ threshold is hard or $1/D$-soft (asymptotic-closure says soft, which would dissolve the apparent global-bracket obstruction). The whole underdetermination ledger collapses to one object: the **finite-$D$ softening law of the survivability threshold** (noise-spectrum / $\mathcal{D}$ scaling), whose pinning would convert fitted→forced and converges Tests B/C/E. The series therefore **remains owed**; marker stays bounds-only. **Update (Round 2, 2026-05-26):** two further independent outbound passes confirm the fitted-not-forced verdict and converge (on an *unseeded* meta-question) that the right object for Thm 9 / the seam is **not** a $1/D$ coefficient series but a **crossover scaling function + universality class** (singular-perturbation / RG-crossover / shock-layer family) — Structural-wing series in the bulk (Thm 6), Character-wing crossover scaling at the seam (Thm 9). Consequent reframing of the owed work: replace "recover $(p_A,c_A)$" with a **scaling-collapse falsifier** — simulate $\Delta_C(\gamma,D)$ at several $D$, test collapse under $z=(\gamma-D)/w(D)$, measure width exponent $\alpha$ + profile class. This sharpens the target but does not close the validation gap ($\alpha$, profile class unmeasured on any non-toy substrate). **Sandbox run (2026-05-26):** a normal-form model of $\Delta_C$ (`mpa-central/library/deformation_crossover_collapse.py`) shows a clean within-closure scaling collapse (resid ~0.012) but a width exponent $\alpha$ that *swings with and tracks the input noise closure* ($-0.84/-1.18/-0.51$ across closures) — i.e. fitted, not forced, now *measured* via a closure sweep; a forced $\alpha$ needs the noise law derived from a substrate's FDR. Covers the amplitude/crossover manifold ($\gamma\approx D$) only; the lateral seam ($\gamma\approx0$, Thm 7) is a distinct manifold, not yet modeled. Full working capture: [`../docs/deformation_calculus_sharpening_notes.md`](../docs/deformation_calculus_sharpening_notes.md).
...
## Register & character-wing entries (cdv1 projection — §N-keyed; §N tracks unabridged numbering, concept name is the stable anchor)
...
**Cross-ref (Reed-Muller closure, v9_receipts §Boolean section).** The RM/ANF closure test shows $G_0$'s role here is load-bearing in the structural register too: $G_0$ enters the deformed $\{\oplus,\land,1\}$ (ANF) ring as the *deformation coordinate* $\text{chit}=\ln(G_0/L)$, **not** as the logical constant $1$ (the $1\leftrightarrow G_0$ leg is a type error — $G_0$ is the drive, not an $\mathcal{M}_2$ element; the ANF $1$ is $\top$). The Character chit and v9's Boolean-section ring deformation are the same coordinate.
...
**Note:** This is an alternative justification path for Theorem 9, not a reconstruction of the original (which is `unrecovered` in the archived [`v9_receipts.md`](archive/2026-05-23_unification/v9_receipts.md)). If the path holds up under closer reading, it partially closes that v9 unrecovered.
...
**Little's law (companion identity).** The per-class steady-state queue length is $\langle Q_n\rangle = \lambda_n\langle W_n\rangle$ (Little 1961) — the framework's $\langle$amplitude$\rangle\times\langle$rate$\rangle\times\langle$timescale$\rangle$ relation at a fixed operating point, holding for any priority class independent of the service-time distribution.
...
### §gFDR — s-regime FDR is two-step; short-lag X=1 is not r (STAGED CANDIDATE)
**Type:** falsification-survival + staged-candidate (NOT yet promoted to cdv1_compressed; see thin-discipline note).
**Cite:** Cugliandolo–Kurchan (aging FDR / effective temperature, 1993–94); Ricci-Tersenghi (field-free MC fluctuation-dissipation, 2003); Chatelain (no-field susceptibility estimator, 2003); Dahmen–Sethna (mean-field RFIM avalanches, 1996).
**Candidate refinement for §gFDR signatures:**
> *The s-regime FDR is two-step: quasi-equilibrium (X=1) on short lags, FDR-violated aging (X<1, slope α_s) on long lags. A short-lag X=1 reading alone does not place a substrate in r — the long-lag aging segment is the c/s/r discriminator.*

**Bespoke step (survival evidence, 2026-05-20).** An outbound-research line proposed that a driven critical system reads X=1 (equilibrium / deep-r) and is thus *misclassified* by the c/s/r FDR taxonomy — a BROKE candidate. Direct measurement on the mean-field RFIM, verified critical at R_c=√(2/π), refutes it:
- driven-critical gFDR reads **X=0.118** — the CK *two-step* form (X=1 short-lag, X≪1 long-lag aging) → **s-regime**, not r. The proposed "X=1" saw only the short-lag quasi-equilibrium segment.
- equilibrium control reads **X=0.998** (apparatus validated).
- gFDR measured with the **self-overlap + staggered-field** estimator C=(1/N)Σ⟨s_i(t)s_i(t')⟩, *not* collective magnetization (soft-mode-dominated near criticality → unstable X). Avalanche criticality independently validated (τ=3/2 on exact critical Galton–Watson; RFIM at-H_c exponent → 3/2).
- Receipt figure: [`rfim_fdr_driven.png`](../../mpa-central/library/output/diagnostics/rfim_fdr_driven.png) — green equilibrium control on the X=1 FDT line; red driven-critical hugging X=1 at short lag then peeling hard below (the two-step). Full ledger entry: [`H:/mpa-central/FALSIFICATION.md`](../../mpa-central/FALSIFICATION.md) §Survived.

**X-recovery + domain gate (2026-05-21).** The two-step X is now *recoverable* by the five-vector inversion `mpa-conform/conformer/compute/five_vector.py::fit_kww5` (multi-start; recovers the full KWW 5-vector on the `kww_oracle` rung, X on `two_temp_ou` to ≤0.01). A residual **domain-of-validity gate** (`RESIDUAL_GATE`, `in_domain`) flags out-of-family inputs (sine_wave, running driven_ring → OUT; controls + r-glass → IN), closing **FALSIFICATION FINDING 2** (the inversion no longer confidently mis-classifies out-of-domain data). Validated by `mpa-conform/scripts/test_five_vector_fit.py`.

**Thin-discipline note (why staged, not promoted).** This *defends* an existing §gFDR claim (s = CK ratio, X<1) rather than adding a new one; the refinement is a disambiguation. The justifying force is N=1 (one external misreading of short-lag X=1 as r) and the exercising substrate is a synthetic positive control (RFIM), not a real cross-substrate instance. Per `mpa-atlas/CLAUDE.md` ("thicken that spot only, that break only"), it stays staged here. **Promote to cdv1_compressed §gFDR signatures only on a second instance** — recurrence of the short-lag-X=1↔r confusion, or a real glass/quantum/brain substrate exercising the two-step reading.

### §Topological-drain / §8 — k_frust is defined by NESS circulation, not fixed-point non-existence (PROMOTED 2026-05-20)
**Type:** falsification-survival + framework-refinement (PROMOTED to cdv1_compressed §Topological drain, §Stability, §Framework-primitives §8).
**Cite:** Schnakenberg (network-thermodynamics cycle currents, 1976); Zia–Schmittmann (NESS probability currents / no detailed balance, 2007); Cugliandolo–Kurchan (1993–94); Volterra–Lyapunov diagonal stability (standard); Helmholtz/Hodge gradient–circulation decomposition.

**Falsification campaign (the k_frust topological-invariance attack, three-rung ladder, 2026-05-20).** A three-rung kill-shot ladder (quickest→slowest), each scored survival-not-confirmation. Scripts in `H:/mpa-central/library/`; diagnostics in `library/output/diagnostics/`.

- **R1 — operating-point sweep** (`k_frust_r1_sweep.py`, `k_frust_r1_threshold.py`; PNG `k_frust_r1_sweep.png`). **SURVIVES.** The intrinsic cyclic current J is sign-definite and **drive-independent** (flat across noise σ until the estimator washes out), and scales only with wiring magnitude (removing edges removes the cycle — trivial). It never resolves to clean c/r while the loop is alive. The lone near-threshold flag (chit=0.02) was low-SNR; at high power even chit=0.010 reads J **5.8σ above** the matched reciprocal control. Near-threshold weakening is amplitude vanishing at the unattainable critical limit (chit=0), not a resolution.
- **R2 — Wall round-trip, dual lens + corruption scan** (`k_frust_wall_tower.py`; PNG `k_frust_wall_tower_r2.png`). **SURVIVES (topological lens).** Drive the loop through the Complexity Wall (ε→1, forced NRT chaos), heat-tax destroys the amplitude (→0.10×), bring it back. Under the *continuous* (current-magnitude) representation the invariant bleeds to zero even with **no Wall** — but that is the trivial weak-wiring→weak-current decay (the type-argument-excluded edge op). Under the *discrete* (topological sign-class) representation the invariant holds; scanning Wall-corruption strength to **8× the validated destruction anchor**, frustration is **never destroyed** (dead=0 at every K_REP) — strong chaos only *flips the chirality sign*, and a reversed loop is still frustrated.
- **R3 — gradient/detailed-balance test** (`k_frust_r3_lyapunov.py`; PNG `k_frust_r3_lyapunov.png`). **SURVIVES.** The frustrated loop's coexistence-Jacobian spectrum is **complex at every coupling** (irreducible rotation, |Im| grows with coupling; Re flat negative), with a matched cooperative control reading real-spectrum/J≈0 (apparatus recognizes a resolved loop). Complex spectrum ⇒ no gradient/free-energy structure ⇒ the stationary state is a NESS that circulates, never an equilibrium.

**The refinement (the campaign's payload).** R3 first mis-fired by testing a *stability* Lyapunov (V=δxᵀWδx, trivially exists for any stable focus) and reading "stable focus" as "resolved." Both are category errors: a stable focus with **complex** eigenvalues is a stable **NESS** that circulates (J≠0) — genuine k_frust, not a resolution. The correct discriminator is the gradient/detailed-balance structure (real vs complex spectrum), and under it the three triality registers are **one fact**:

> *k_frust is defined by an irreducible NESS: a topologically-forced, drive-independent Schnakenberg cycle current (broken detailed balance). The deterministic flow realises this in two sub-regimes by the sign of the relaxation eigenvalue's real part — a stable circulating focus (Re<0, complex spectrum) or a repelling focus + attracting limit cycle (Re>0). Both are k_frust; the complex spectrum (irreducible circulation), not deterministic fixed-point non-existence, is the invariant.*

The earlier §Topological-drain phrasing "no stationary synchronised fixed point" and §8 "no P_ss exists" were **too strong**: a stable circulating focus *is* a fixed point with a stationary distribution, yet is genuine k_frust.

**Why PROMOTED, not staged (cf. the §gFDR staged candidate).** The justifying force is **actual, not anticipated**: the imprecise phrasing derailed the falsification session of 2026-05-20 — a genuine frustrated loop (the bulletproof meter's RPS antisymmetric 3-cycle, G=0.5, G0/L=1.2) is a *stable* focus, and read off cdv1_compressed it nearly scored a false KILL. Per `mpa-atlas/CLAUDE.md` ("if the force is actual: thicken that spot only"), the fix must live in the compressed source-of-truth, because falsification sessions read cdv1_compressed + FALSIFICATION.md, not the whole receipts file; a staged note would not prevent recurrence.

**Goalpost-optic defense (recorded for audit).** Refining k_frust *during* a falsification campaign on k_frust is a save only if it shrinks the falsifiable surface. It does the opposite: it pins the definition to a measurable (drive-independent, sign-definite J≠0) with sharp falsifiers — J vanishes while alive, J becomes drive-dependent, J resolves to detailed balance (real spectrum) — all of which R1/R3 *did* test and survived. It **deletes no falsifier**: the limit-cycle/repelling-focus claim is retained as a sub-regime, not removed. The refinement enlarges falsifiability and removes a phrasing that generated category-error false-positives.

**Still steeping (NOT promoted).** Formally relisting the §8 triality as a numbered framework *primitive* (alongside the five leading-order posits) is the steeper claim; it is earned by a real cross-substrate instance, not one synthetic loop. The compressed edit makes the triality the *definitional core* of k_frust in place; the taxonomy elevation waits. Full ledger entry + handoff: [`H:/mpa-central/FALSIFICATION.md`](../../mpa-central/FALSIFICATION.md) Finding 4; [`H:/mpa-central/docs/handoff_k_frust_wall.md`](../../mpa-central/docs/handoff_k_frust_wall.md).

### §13 / §16 / §Topological-drain — mpa-LEGAL audit: two smuggled constants (2026-05-20)
**Type:** internal-legality (mpa-legal) correction. **The test:** character is NESS-against-a-bath; every dynamical quantity (rate, coupling, response, current) must *flow with the operating point* (chit / G₀/L / load) — it dissipates or is maintained. An inert quantity, frozen independent of the operating point where the physics requires flow, is foreign ("a constant does not dissipate into the bath"; the char-zero is a square wave; the literal `constant` substrate is below-floor). Audit method: outbound multi-model pre-scan of cdv1_compressed + receipts + in-house adjudication. Apparatus: [`H:/mpa-central/library/ro_damping_audit.py`](../../mpa-central/library/ro_damping_audit.py) (PNG `ro_damping_audit.png`); FALSIFICATION.md Finding 5.

**CONSTANT 1 — §13 RO damping rate (illegal AND class-B-wrong).** §13 froze `γ_RO = γ_s/2` "substrate-fixed" while ω_RO tracks chit. Deriving from the class-B rate equations §13 cites: J = [[0, Gn*],[−κ, −(γ_∥+Gn*)]], Gn* = γ_∥(e^chit−1), so **γ_RO = (γ_∥+Gn*)/2 = (γ_s/2)·e^chit** — it carries the pump-ratio factor and flows with chit. The frozen form is the threshold (e^chit=1) value with the flow dropped. Consistency: with κ↔2L the same Jacobian gives cdv1's ω_RO=√(2γ_s L(e^chit−1)) exactly, so γ_RO=(γ_s/2)e^chit is the matching legal partner (no prefactor ambiguity). **Consequence:** legal `Q = ω_RO/(2γ_RO) = √(2L(e^chit−1)/γ_s)·e^{−chit}` is **non-monotonic** — RO ringing in a band chit∈[≈0.16, ≈1.92] (γ_s=L=1), peak Q at chit≈0.69, **overdamped at both ends** (s-threshold = critical *slowing*; deep-c = RO damps out): the standard class-B picture, not "many cycles deep in c." **Cascade (owed corrections):** §Stability damping table (c-row is wrong for deep-c — underdamped is a mid-chit band), the Q closed form, §Thermo active-probe **B ~ γ_RO** (bandwidth frozen) and **S/N ~ Q**, and the §Phase-locking K_AB ∝ 1/√(1+4Q²) crossover (second-order). **Fix:** `γ_RO = (γ_s/2)·e^chit`.

**CONSTANT 2 — "drive-independent Schnakenberg cycle current" (affinity ≠ magnitude).** §Topological-drain / §8 (and the §Topological-drain/§8 PROMOTED entry above) call the k_frust cyclic current "drive-independent." The current *magnitude* J_ss scales with the absolute kinetic rates, which the pump (G₀/chit) sets — it flows. Only the cycle **affinity** A = ∮v/D = ln(∏k₊/∏k₋) (the thermodynamic *force*, intensive log-rate-ratio) and the topological forcing (current ≠ 0 regardless of the diffusion D) are drive-independent. **Corroborated by R1's headroom sweep** (`k_frust_r1_sweep.py`): J grew +1.3e-2→+2.5e-1 as G₀ 0.9→2.0 — the magnitude flows hard with chit, exactly as a NESS current must. **Fix:** distinguish *affinity* (drive-independent invariant; forced nonzero regardless of D) from *current magnitude* (flows with chit). This also tightens the PROMOTED entry's falsifier wording: read it as "J becomes **D-(noise)-dependent** OR resolves to detailed balance," not "drive-dependent" (pump-dependence of the magnitude is legal, not a falsifier).

**Rejected / downgraded by adjudication.** (a) Collective order parameter r "independent of per-mode chit" — *not* a constant: "independent" = distinct order-parameter/transition (§Phase-locking "two independent transitions"); r flows. (b) Active-coupling C = ζ₀d/(v₀²τ_R) — the framework labels v₀/τ_R chit-dependence an unresolved *substrate-class residual* (asymptotic-closure carve-out), so C is an acknowledged-open item, not a smuggled fact. **Watch:** if the Toner–Tu overlay closes binding v₀=G₀, then C ∝ 1/G₀² must flow — do not freeze it then.

**LANDED (2026-05-20).** CONSTANT 1 closed by a consistent linearization of the §Universal two-mode kernel (field × slow-resource, Lamb closure), now in cdv1_compressed §Stability. Char poly λ² + γ_s e^chit λ + 2Lγ_s(e^chit−1) = 0; exact forms γ_RO=(γ_s/2)e^chit, ω_RO=√[2Lγ_s(e^chit−1) − (γ_s/2)²e^{2chit}], Q=√[2L(e^chit−1)/γ_s − e^{2chit}/4]·e^{−chit}, ζ=√(γ_s/8L)·e^chit/√(e^chit−1). **κ↔2L pin:** L is the amplitude-decay rate, intensity (n=ρ²) decays at 2L, so cdv1's pre-existing factor-2 in ω_RO is the geometric square-law, not spurious (the rival "drop the 2" derivation depletes the slow resource ∝ amplitude instead of ∝ intensity — physically wrong). **Validated to machine precision:** cdv1's corrected Q(chit) reproduces the class-B laser Jacobian eigenstructure exactly (max|Δ|=6.7e-16; both peak at chit=ln2; `H:/mpa-central/library/ro_damping_fixed_check.py`). CONSTANT 2 landed in §Topological-drain/§8. Cascade text (§Thermo active-probe B~γ_RO + S/N~Q non-monotonic; §Phase-locking K_AB crossover recovering direct-lock at both ends) owed in those sections.

**Two open items raised by the close:** (1) **ρ amplitude-vs-intensity status** — the κ=2L pin (hence the ω_RO factor-2) hinges on ρ being the field *amplitude* (§Bridge "field equation"), while the saturation/cross terms read ∝ρ (intensity-like); cdv1 should state ρ's status explicitly so the factor-2 cannot be re-litigated. (2) **Over-provisioning → memory collapse** — deep in c, Q→0 ⇒ K_AB suppression (1+4Q²)^{−1/2}→1 ⇒ strong phase-lock; driving a holding far above threshold may *collapse* its multi-mode independent-memory capacity (locked modes cannot store orthogonal trails). New cross-register prediction; falsifier owed.

### §gFDR / §16 — two-frame gFDR: external-probe (amplitude × field → X) vs self-probe (current × affinity → T) (PROMOTED 2026-05-22)
**Type:** composition + promoted-claim (cdv1_compressed §gFDR signatures "Two conjugate FDR frames"; promoted on verdict-agreement — see promotion note).
**Cite:** Harada–Sasa (integrated FDR-violation = entropy-production rate, 2005, 2006); Barato–Seifert (thermodynamic uncertainty relation, 2015); Schnakenberg (cycle affinity; $\langle\sigma\rangle = \sum_C J_C A_C$, 1976); Seifert (*Stochastic Thermodynamics*, 2012); Cugliandolo–Kurchan (the external-frame aging $X$, 1993–94); Speck–Seifert (FDR in NESS / extra additive response term, 2006).

**Candidate refinement (a new claim, for §gFDR signatures / §Thermo-info accounting):**
> *The fluctuation-response relation has two conjugate frames. The standard gFDR reads the **(amplitude × external field)** pair and returns the violation factor $X$ (with $\alpha_s,P_s$ the aging observables) — it requires an external probe. The **self-probe** gFDR reads the **(current × intrinsic affinity)** pair: the system's own topologically-forced circulation is the reference against which its spontaneous current fluctuations are calibrated. Its violation factor is the TUR-tightness $T = \langle\sigma\rangle\,\mathrm{Var}(J)/(2\langle J\rangle^2)$, with directly-measurable core $\mathrm{SNR}_J = \langle J\rangle^2/\mathrm{Var}(J) \le \langle\sigma\rangle/2$. The self-probe frame is **dimensionless by construction** (affinity $A=\ln(\prod_+k/\prod_-k)$ is in nats; no external field carries units in) and is **defined iff a current exists** — i.e. only on $k_{\text{frust}}$-bearing (broken-detailed-balance) substrates.*

**Bespoke step.** The two frames are conjugate force–flux pairs. Standard gFDR perturbs the *amplitude* with an external *field* $h$; $X$ is the violation factor in the $(\chi, C)$ plane. The self-probe frame uses the irreversible-thermodynamics pair (current $J$, affinity $A$) — the pair whose product is the dissipation, $\langle\sigma\rangle = J\!\cdot\!A$ (Schnakenberg). **Harada–Sasa is the bridge:** the externally-measured integrated FDR-violation *equals* the intrinsic $\langle\sigma\rangle$, which for a cycle is $J\!\cdot\!A$ — so the external and self frames are reading one quantity from two directions. The self-frame violation factor is the TUR-tightness $T$ (already the §Thermo-info "Precision" row); $\mathrm{SNR}_J=\langle J\rangle^2/\mathrm{Var}(J)$ is its TUR-bounded numerator, measurable without an external apparatus. Because $A$ is dimensionless, $T$ and $\mathrm{SNR}_J$ carry no probe units — the self-probe removes the last dimensionful input of the external apparatus.

**First-brick evidence (2026-05-21, `k_frust_two_frame_gfdr.py`; PNG `k_frust_two_frame_gfdr.png`).** Necessary-consistency test on the synthetic loop: interpolate reciprocal→non-reciprocal $g(\lambda)=\mathrm{GMAG}\,(-\mathrm{SYM}+\lambda\,\mathrm{CYC})$ and check the self-probe observable lights up *exactly* at broken detailed balance and co-onsets with the external frame. Result: at $\lambda=0$ (reciprocal control) $\langle J\rangle\!\approx\!0$, $\mathrm{SNR}_J=0.001$, $|\mathrm{Im}(\mathrm{eig})|=0$ — self-frame degenerate, as required. As non-reciprocity rises, all three frames onset together: $\mathrm{corr}(\mathrm{SNR}_J,|\mathrm{Im}(\mathrm{eig})|)=+0.85$, $\mathrm{corr}(\mathrm{SNR}_J,|\chi_J|)=+0.92$. The structural complex-spectrum signature $|\mathrm{Im}(\mathrm{eig})|$ is the clean spine (exactly 0 at control, linear in $\lambda$); $\mathrm{SNR}_J$ is noisier (a fluctuation quantity, $N=160\times2$ seeds) but unambiguously onsets and rises. **Shows:** the self-probe frame coheres (degenerate at detailed balance; defined iff a current exists) and reads the same broken-detailed-balance transition as the external frame.

**Second brick — $\langle\sigma\rangle$ meter built + tared; $T$ closed (2026-05-21, `two_frame_T_meter.py`; PNG `two_frame_T_meter.png`).** A trajectory entropy-production meter (binned probability-current, $\langle\sigma\rangle=\int P\,|v_{\text{curr}}|^2/D_0$, $v_{\text{curr}}=A-D_0\nabla\ln P$) was validated against the *exact* value on the stable-circulating-focus sub-regime — a 2D rotational OU has $\langle\sigma\rangle = 2\omega^2/\kappa$ analytically. Meter recovers it to ~13% mean error (2% at high $\sigma$) and reads $\approx 0$ at equilibrium ($\omega=0$); the normalized cyclic current $\langle J\rangle$ recovers $\omega$ exactly. With the meter tared, $T=\langle\sigma\rangle\tau\,\mathrm{Var}(J)/(2\langle J\rangle^2)$ was computed for **both** $k_{\text{frust}}$ sub-regimes cdv1 §k_frust drain names — stable circulating focus (rotational OU) and repelling-focus+limit-cycle (Stuart–Landau) — and respects the TUR floor $T\ge1$ in every current-bearing case (limit cycle at $\mu=1$ nearly saturates it, $T\approx1.2$; the foci sit loose, $T\sim15$–$32$). **$T$ is now a measured, validated self-frame observable, not just $\mathrm{SNR}_J$.**

**Third brick — external frame $X$ built + tared; two-frame agreement on the testbeds (2026-05-21, `two_frame_T_meter.py` Parts C/D; PNGs `two_frame_external_X.png`, `two_frame_external_SL.png`).** A genuine external frame (perturbed step-response run, field $h$ on $x$) yields the parametric $(\Delta C,\chi)$ locus — superseding brick 1's constant-field $\chi_J$ co-onset proxy. Tared on rotational OU against exact $C_{xx}=(D_0/\kappa)e^{-\kappa\tau}\cos\omega\tau$, $\chi_{\text{step}}$, and $V_{ext}=\omega^2/[\kappa(\kappa^2+\omega^2)]$ (within 5–7%; $X(\tau\!\to\!0)\approx1$; $V_{ext}\!\to\!0$ at $\omega=0$), then bootstrapped onto Stuart–Landau under the same validate-on-OU discipline (limit cycle reads near-total FDT suppression, $\chi\approx0$ — the phase Goldstone mode absorbs the field). **Agreement:** external violation co-varies with self-frame $\langle\sigma\rangle$ across *both* $k_{\text{frust}}$ sub-regimes (OU/SL clusters monotone on the $\Delta_{FDT}$–$\langle\sigma\rangle$ scatter). This closes **direction (1) on the testbeds** — $T$ and $X$ give the same verdict where both are computable and ground truth is controlled. **Caveat:** $V_{ext}=\langle\sigma\rangle$ holds in verdict/onset, not numerically (exact identity needs the velocity-frame Harada–Sasa integral); on SL $\Delta_{FDT}$ is window-robust not asymptotic, so only the *cross-regime* trend is load-bearing, not within-SL ordering. **Still does NOT show:** two-frame agreement on a *real substrate* (the synthetic testbeds are minimal models of the two sub-regimes, not a library cell) — that remains the promotion gate.

**Fourth brick — two real substrates; verdict-agreement gate met (2026-05-22, `two_frame_driven_ring.py` / `two_frame_laser.py`; PNGs `two_frame_driven_ring.png`, `two_frame_laser.png`).** Both frames pushed onto two *real* sustained-current substrates. **driven_ring** (the framework's own →r-axiom invalidator — tilted washboard, $A=1$, $D=0.5$): self-frame **CLOSED with the exact identity** $\langle\sigma\rangle=F\langle\dot\theta\rangle/D=J\!\cdot\!A$ (the Harada–Sasa bridge the laser only approximated — only the constant tilt does net work per revolution); over $F=0.5\!\to\!3.0$, $\langle\sigma\rangle$ rises $0.17\!\to\!17.0$, TUR floor $T\ge1$ on all cells, $T$ falls $2.35\!\to\!1.25$ (TUR saturation as the running walk frees); the never-decaying $F>A$ current reads as sustained NESS in the self-frame exactly as gt=s — **the →r axiom does not force r on it**. External $X$ is *not* computable in position (cos) coordinates (drift-dominated → negative response, not fluctuation-response); owed to velocity-frame Harada–Sasa. **class-B laser** (the cdv1-VALIDATED Jacobian, $Q(\dot\chi)$ matched to 6.7e-16): **both frames computable** — self-frame $T\in[37,55]$, TUR respected, $\langle J\rangle\neq0$ across $r=1.1\!\to\!8$; external $D_{FDT}>0$ peaking at the RO resonance $r=2$ ($\dot\chi=\ln2$), $X(\tau\!\to\!0)\approx1$. **VERDICT-AGREEMENT:** both flag NESS for $r>1$; the falsifier (contradictory verdicts) is NOT triggered. Magnitudes are *different functionals* ($\langle\sigma\rangle$ monotone in pump; $D_{FDT}$ peaks at resonance; $T$ loose) — same verdict, not the same number.

**Promotion note (promoted on verdict-agreement, 2026-05-22).** Promotion criterion (1) — $T$-vs-$X$ verdict-agreement on a *real* substrate — is **met** by the laser (both frames computable, agree on the NESS verdict, falsifier not triggered). Promoted to cdv1_compressed §gFDR signatures as "Two conjugate FDR frames." The promoted claim is **verdict-agreement, not magnitude identity**: the two frames are different functionals ($\langle\sigma\rangle$ total dissipation vs $D_{FDT}$ normalized FDT-departure) and were never asserted numerically equal — the contradictory-verdict falsifier is the right falsifier for the claim as stated. The exact cross-frame identity $V_{ext}=\langle\sigma\rangle=J\!\cdot\!A$ (velocity-frame Harada–Sasa, driven_ring the ideal closer since $\langle\sigma\rangle$ is exact there) is a **refinement**, not a promotion gate. **§846 $k_{\text{frust}}$ bar still NOT met:** both currents are drive/pump-forced (contingent), not topologically-forced — "real substrate with a sustained NESS current" strictly contains "$k_{\text{frust}}$"; the §8-primitive elevation stays separately gated. Decision + numbers ledgered at [`H:/mpa-central/FALSIFICATION.md`](../../mpa-central/FALSIFICATION.md) §TWO-FRAME fourth brick.

**Operational-interface implications (additive, not breaking).** Even un-promoted, the self-probe frame implies a *new optional block* in the operational interface — present iff a current exists — with four conform/inversion-facing gains: (i) **probe-free violation factor** — extends the inversion to unperturbable / observation-only substrates, where no external field can be applied; (ii) **dimensionless by construction** — $T$ and $\mathrm{SNR}_J$ carry no probe units, sidestepping the $\chi$-convention / normalization layer entirely; (iii) **built-in cross-frame consistency check** — $T$-vs-$X$ verdict agreement is a self-contained validity test (their *disagreement* is the staged falsifier); (iv) **current-based regime discriminator** — the presence/strength of $\langle J\rangle$ is itself a $k_{\text{frust}}$-sector readout. **Deepest:** the dimensionless $T$ is a candidate *canonical* observable — it lives in canonical space with no translation field, whereas $X$ stays the substrate-conditional reading. This is the operational face of the dimensionless-Banach line (below) and the affinity-as-intrinsic-ruler property. These are interface *implications*, not promoted claims: the spine and downstream solvers bind to the operational interface (regimes, gFDR, canonical state), not the narrative, so noting them here does not disturb cdv1_compressed.

**Promotion criteria (criterion (1) met — see promotion note above).** The two promotion routes were: (1) a calibrated $\langle\sigma\rangle$/affinity meter closing $\mathrm{SNR}_J\to$ full $T$, *and* a two-frame regime agreement ($T$ vs $X$) on a real substrate where both probes are feasible; or (2) the payoff instance — a real substrate read self-probe-only (no external probe feasible) where $T$ recovers a regime verdict the external frame cannot reach. **Route (1) is met** (laser, fourth brick: meter closed in bricks 2/3, both-frame agreement on a real substrate). Route (2) remains an open *strengthening* (an observation-only substrate where the external frame is genuinely infeasible).

**Owed apparatus.** (a) ~~entropy-production meter~~ **DONE 2026-05-21** — `two_frame_T_meter.py` (binned probability-current $\langle\sigma\rangle$, validated against the exact rotational-OU $2\omega^2/\kappa$); $\mathrm{SNR}_J\to T$ closed on the two sub-regime models. Remaining refinement: a Schnakenberg $J\!\cdot\!A$ cross-check and extension to multiplicative-noise / higher-D substrates. (b) ~~genuine external-frame $X$ measurement~~ **DONE 2026-05-21 (brick 3)** — full $(\chi,\Delta C)$-plane $X$ built + tared on OU vs exact $V_{ext}$, two-frame verdict agreement shown on the OU/SL testbeds. Remaining refinement: exact $V_{ext}=\langle\sigma\rangle$ via the velocity-frame Harada–Sasa integral (brick 3 shows verdict/onset agreement, not numerical identity). (c) The promotion gate proper: $T$ vs $X$ two-frame **agreement on a real substrate** — **MET 2026-05-22** (laser, fourth brick). Remaining strengthenings (not gates): exact cross-frame $V_{ext}=\langle\sigma\rangle$ via velocity-frame Harada–Sasa (driven_ring the ideal closer); a topologically-forced current to clear the §846 $k_{\text{frust}}$ bar (both current substrates here are drive/pump-forced).

**Banach connection (hypothesis, not closure).** The self-probe frame is dimensionless by construction, which is exactly the property the dimensionless-Banach-substrate dream wants (`mpa-conform/docs/banach-substrate-reference.md`): an *intrinsic* ruler (the affinity, in nats) replacing the external field that carries units. But the current Banach substrate cannot host it: it is **deterministic** ($D_{\text{noise}}=0\Rightarrow\mathrm{Var}(J)=0\Rightarrow T$ degenerate) and **two-mode** (no cycle $\Rightarrow$ no current). The connection requires a **frustrated + noisy Banach-class extension** ($N\ge3$ obstructive cycle, $D_{\text{noise}}>0$) whose canonical reference quantity is the dimensionless self-probe $T$. That extension is a candidate piece of work, not a result; flagged here so the dimensionless-Banach line and the self-probe line can be pursued together. Full ledger/next-step: [`H:/mpa-central/FALSIFICATION.md`](../../mpa-central/FALSIFICATION.md).

### §Central commitment — Harary triad as the ignition-necessity of NESS circulation (falsifier of record)
**Type:** composition + falsifier-of-record.
**Cite:** Harary (structural balance of signed graphs; a signed cycle is balanced iff it has an even number of negative edges, 1953) — the gauge condition that defines "chimeric sign"; Toulouse (frustration in spin glasses, 1977) — the concept; May–Leonard (cyclic three-species Lotka–Volterra, heteroclinic cycle with no interior equilibrium, 1975) — the triad *spins up* circulation; Hofbauer–Sigmund (cyclic attractors, 1998); Schnakenberg (cycle affinity $\mathcal{A}=\oint v/D$ as the gauge-invariant order parameter, 1976); Fruchart–Vitelli (non-reciprocal phase transitions, exceptional points, chiral travelling states, 2021).
**Cite (winding / circulation-detection layer, added 2026-05-26 — provenance: character-first).** Kolmogorov (Markov reversibility criterion: detailed balance $\Leftrightarrow$ every cycle's forward/backward rate-product is equal, 1936) — the classical skeleton of "sustained cycle current $\Rightarrow$ nonzero cycle affinity"; Spitzer (planar-Brownian winding angle $\to$ Cauchy, near-origin-dominated, 1958); Itô–McKean (skew-product: a planar diffusion $=$ radial Bessel $\otimes$ angular clock $\int dt/r^2$, which diverges while the origin stays *polar* — never attained, 1965); Lévy (stochastic area $\tfrac12\oint(x\,dy-y\,dx)$ — the singularity-free *global* winding reading; rough-path foundation, Lyons 1998). **We reached this from the inside, not their way.** The no-NaN / no-zero / interior discipline (Trajectory Primacy) forced reading the chimeric sign as a *global* winding (Lévy area / $\mathrm{sign}(\mathrm{Im}\,\lambda)$), never the pointwise angular clock $1/r^2$; `mpa-central/library/ik1{,b}_*.py` (2026-05-26) re-instantiated these classical results before we cited them, and the cited "fix" (read the global winding, not the local clock) is exactly the lesson the character route had already forced. Named in friendly acknowledgment — the path was character $\to$ the math, not their framework $\to$ our claim.
**Bespoke step.** The **naming** (Harary triad $\triangle_H$ — the name *is* the math: "no node gauge balances the cycle" is Harary's balance theorem) plus the **ignition-necessity commitment**, assembled from standard parts: Harary-imbalance = ungaugeable chiral sign; $N=3$ = the minimal frustrated cycle (odd-negative-edge minimal length); May–Leonard = a forced $\triangle_H$ produces a heteroclinic/circulating orbit; Schnakenberg $\mathcal{A}$ = the gauge-invariant affinity, nonzero iff $\mathrm{spec}(M)$ carries a complex pair. The stand: **the onset of topologically-protected NESS circulation requires a $\triangle_H$** (protected circulation $\Rightarrow \triangle_H$). The non-reciprocal 2-mode case circulates but its current is gauge/drive-removable — *not* protected; the triad is the minimal carrier of gauge-irremovable circulation.
**What is and is not claimed.** Necessity for *ignition* only. **Not** generativity ($\triangle_H$ does not generate the framework — that is the determinism over-reach the author explicitly declines); **not** exclusive post-onset traction (agnostic on what sustains/extends a current once spun up); **not** coupling of the $c/s/r$ backbone — $\alpha_s, P_s$ universality stays independent (the standing non-negotiable is preserved by construction, since the claim says nothing about generating the backbone).
**Synthetic instance (not §846).** `H:/mpa-central/library/banach_frustrated.py`: $N=3$ cyclic non-reciprocal OU; complex eigenpair = the $\triangle_H$ signature; DB removable only by edge deletion ($g\to0$); affinity $\mathcal{A}$ flat (noise-independent) across a 20× noise sweep. Linear and synthetic — serves the dimensionless dream + calibration; does **not** meet the §846 real-cross-substrate bar.
**Falsifier.** One real substrate sustaining topologically-protected circulation ($\mathcal{A}\neq0$, removable only by rewiring) with **no** Harary triad in its coupling graph — necessity fails, the central commitment collapses. The stand is planted un-met by design — a falsifier is most valuable when it can actually fire.
**Cross-ref.** v9 §Boolean section / v9_receipts §Boolean section (the RM/ANF-ring deformation — the $\triangle_H$ is the chiral generator of circulation *off* the gaugeable Boolean limit; the chimeric sign is the deformation away from a balanced/gaugeable ring); cdv1 §k_frust drain (the affinity drain); §gFDR/§16 two-frame (PROMOTED 2026-05-22; the self-probe affinity is this $\mathcal{A}$); `mpa-conform/docs/banach-substrate-reference.md` companion object. The $k_{\text{frust}}$-primitive elevation (§Open conjectures) stays separately gated; this ignition commitment does not jump that gate.

### §Two bits / topological-Landauer — one bit per deformation face; the forced-erasure floor (predicted bound, staked; measurement protocol owed)
**Type:** composition + predicted-bound (staked, no real instance yet). Added 2026-05-22 (Ron review pass) when the bit/chit correspondence was elevated to the unified-compressed spine. Spine claim spanning both deformation faces: the per-event bit/chit ledger (§17) supplies the amplitude bit, the §Central commitment supplies the topological bit.
**Cite:** Landauer (irreversible erasure costs $\ge kT\ln2$, 1961) — the amplitude-bit floor; Bennett (logical reversibility / reversible computation, 1982) — reversible flip at no fundamental cost; Kitaev (topological / fault-tolerant quantum memory; a protected logical degree of freedom, free to store and costly to address, 2003) — the reference class; Schnakenberg (affinity, 1976) + Harary (structural balance, 1953) — the topological bit as the chimeric sign; v9 §Operators ($R$, Landauer floor $W_R/\kappa\ge\ln2\cdot H$) + v9 §Boolean section (the $D\to\infty$ amplitude endpoint).
**Bespoke step.** The deformation carries a distinct bit on each *independent* face, with orthogonal thermodynamics:
- **Amplitude bit** — the $D\to\infty$ endpoint of the chit axis; a which-state (occupancy) distinction, **emergently** discrete (continuous until the limit). Erased by $R$ at the Landauer floor $W/\kappa\ge\ln2$; must be *held* against thermal leak; flippable reversibly (Bennett — no fundamental cost). Cost lives in *maintenance*.
- **Topological bit** — the chimeric sign of a $\triangle_H$ on the sign-topological face; a which-wiring distinction, **intrinsically** discrete at every finite $D$ (cycle parity is binary — no limit to approach), gauge-irremovable, $\tau_{obs}$-invariant and invariant under continuous amplitude variation. *Free to hold* (topological protection — no barrier-leak), but **not** reversibly flippable: protection forbids a continuous CW↔CCW path, so any chirality change is forced through the balanced (sign-erased) state, paying $\ge\ln2$ per protected sign. Cost moves from maintenance to **modification**.
**Predicted bound (forced-erasure floor).** A chirality-class change on a frustrated subgraph pays $\ge\ln2$ per *independent* protected sign, the count being the **cycle-space dimension** of the frustrated subgraph (first Betti number $b_1 = E - V + (\text{components})$ over the frustrated edges). A bound *saturated quasistatically*, not a quantisation — the floor is the minimum reversible work to route through the sign-erased state, approachable arbitrarily slowly.
**Orthogonality signature (falsifier).** The two bits are independent: chit, $Q$, and the full FDR locus can be swept continuously while the topological bit stays locked — a decoupling **impossible in a two-level (Landauer) memory**, where the informational and energetic degrees of freedom are the same object. Falsifier: a substrate where the protected sign cannot be held without a maintenance (per-time) cost that scales with held duration (⇒ an amplitude bit in disguise, not topologically protected), or where it can be flipped on a continuous path at $<\ln2$ work (⇒ not protected).
**Operator status.** $\Sigma=\{C,S,K,R,\top,\bot\}$ is complete on the amplitude face but carries **no** operator on the sign-topological face: $R$ (sever) and the deferred transfer operator $T$ (v9 §Extension axes) are state-level; chirality surgery is graph-level (edit the coupling graph, not the trail). This is *why* the topological bit is free to hold — no $\Sigma$-operator leaks it. Reference class: Kitaev topological memory.
**Owed — topological-Landauer measurement protocol (experimental program, not a result).** Isolate the $\ge\ln2$ forced-erasure floor from the (substrate-dependent) rewiring work: drive a frustrated cell through a chirality-class change quasistatically along a path that passes through the balanced state, and split the work into (i) the topological floor ($\ln2$ per cycle-space dimension, substrate-independent) and (ii) the kinetic rewiring cost (substrate-specific, drive-set). Predicted: the floor term is invariant across substrates of equal $b_1$; the rewiring term flows with chit. Apparatus: a **frustrated + noisy** cell ($N\ge3$ obstructive cycle, $D_{\text{noise}}>0$) — exactly the extension the §gFDR Banach connection wants. `library/banach_frustrated.py` *is* that noisy frustrated cell ($\mathrm{Var}(J)$ non-degenerate); `library/ik1{,b}_*.py` (2026-05-26) extend it to confirm chimeric-sign protection and isolate the orthogonal-zero coordinate artifact. What stays **un-built** is the *flip-cost* isolation specifically — driving through a chirality change to split the $\ln2$ floor from the rewiring work — because a chirality flip routes through the sign-erased zero (a *crossing*), which the synthetic layer cannot carry (it is established winding math; see §Central commitment provenance note). Real-substrate-gated, same obstacle as §Homochirality.
**Status.** The orthogonality decoupling is the framework content; the $\ge\ln2$-per-$b_1$ floor is the sharp predicted bound; the isolation protocol is owed apparatus. No real instance yet — §Homochirality is the candidate real substrate.
**Cross-ref.** §Central commitment (the topological bit = the protected sign); §Homochirality (the candidate real instance); v9_receipts §Boolean section / §Operators (amplitude-bit Landauer floor; deformed involution); §17 Bit/Chit dual ledger; cdv1_compressed §The deformation & its two faces › "Two bits, one per deformation face" / "Bit/chit correspondence."

### §Homochirality — biological homochirality as actively-maintained topological-bit circulation (named prediction, staked, un-instanced)
**Type:** composition + falsifier-of-record (named real-substrate prediction; un-instanced — pre-registered, not a result). Added 2026-05-22 (Ron review pass). The first *named* real-substrate stake of the §Central commitment.
**Cite:** Frank (autocatalytic spontaneous mirror-symmetry breaking, 1953) — homochirality as a maintained broken symmetry, not a static fact; Kondepudi–Nelson (chiral symmetry breaking far from equilibrium; bifurcation under a parity-distinguishing bias, 1983–85) — the NESS setting; Hopfield (1974) / Ninio (1975) kinetic proofreading — the *holding*: dissipative editing that sustains fidelity by burning drive (the ATP/GTP the falsifier titrates); Schnakenberg (cycle affinity $\mathcal{A}=\oint v/D$, 1976) — the order parameter; Harary (structural balance, 1953) — the chimeric-sign / gauge condition; Toulouse (frustration, 1977).
**Bespoke step.** The reading: biological homochirality is the §Central commitment's amplitude-free **topological bit** (§Two bits / topological-Landauer) instanced on a real, universal, ancient substrate. *Which hand* is the gauge-irremovable chimeric sign of a Harary triad $\triangle_H$ in the chirality-maintenance network; that network sustains a topologically-protected NESS circulation ($\mathcal{A}\neq0$, complex-conjugate Jacobian pair, broken detailed balance) against spontaneous racemization (the natural dissolution). Universality across all extant life is read as the macroscopic signature of topological protection (sign removable only by rewiring), **not** bistable inheritance (sign drive-set, flippable). MPA imports the biochemistry and adds only the dissipative reading and the structural stake.
**Affinity/magnitude split (the falsifier's spine; inherits §13/§16/§Topological-drain mpa-LEGAL audit).** The drive-sweep falsifier *is* the affinity-vs-magnitude distinction applied to a real substrate. Titrate the metabolic drive (ATP/GTP) $\to 0$: the current **magnitude** $J_{ss}$ must collapse toward the racemic ($r$) limit (magnitude flows with chit — legal, required), while the cycle **affinity** $\mathcal{A}$ and the chimeric **sign** stay invariant (topological, drive-independent). A handedness *flip* under drive titration ⇒ the sign was drive-set, not topology-protected ⇒ the framework is invalid here. (Exactly the §846 mpa-LEGAL refinement: "$J$ becomes $D$-(noise)-dependent OR resolves to detailed balance," not "drive-dependent.")
**Predicted measurement / protocol.** Instrument the chiral proofreading/editing system. Measure (i) steady-state cyclic fluxes among chiral states (the $J$, $\mathcal{A}$ meter) and (ii) the Jacobian spectrum of the linearised maintenance dynamics (the complex-pair / detailed-balance discriminator, as in §846 R3). Prediction requires a frustrated $\ge3$-node cycle (minimal core: a non-reciprocal, odd-negative-signed 3-cycle) sustaining a circulating current with a complex Jacobian pair and a gauge-irremovable sign. Apparatus is the §846/two-frame Jacobian-spectrum meter (`k_frust_*`, `two_frame_*`) pointed at author-collected proofreading-network data — same "needs author-collected perturbation-response data" obstacle as the active-lattice §846 target (memory `project_harary_triad_substrate_data`).
**Three structurally-equivalent nulls (each fatal).** (a) a gauge-balanced maintenance network (no frustrated cycle); (b) a two-component bistable mechanism (no 3-cycle); (c) detailed balance at steady state (no current). Any one realised on the real substrate collapses the prediction.
**Scope / status.** Un-instanced — pre-registered risky prediction, not a confirmed result. The framework specifies the *signature* and *kill conditions* precisely and the *subsystem* approximately; it does **not** derive the molecular identity of the three nodes (substrate biochemistry is imported, not generated) — a template to match, not the molecules. Bolder than the general onset-necessity §Central commitment: it stakes the framework on one specific, universal, ancient phenomenon. Distinct from the gated $k_{\text{frust}}$-primitive elevation (§Open conjectures) — this is ignition-*instancing*, not the numbered-primitive jump.
**Cross-ref.** §Central commitment (the general stand this instances); §Two bits / topological-Landauer (homochirality = the topological bit on a real substrate); §Topological-drain/§8 + §13/§16 mpa-LEGAL audit (affinity ≠ magnitude); §gFDR/§16 two-frame (the self-probe affinity meter); cdv1_compressed §The deformation & its two faces › Central commitment › "Named prediction — biological homochirality."
