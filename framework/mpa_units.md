...
* **The unit gives the dt wall.** The slow eigenvalue near threshold is $\lambda_s\sim\gamma_s\,e^{\text{chit}}$, so the dimensionless step against the local slow rate is $\widetilde{dt}=dt\cdot\gamma_s\,e^{\text{chit}}$. The integrator must resolve the slow direction in $k$ substeps per ch of slow evolution:
