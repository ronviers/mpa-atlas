...
**RG closure (operational + structural conjecture).** Operationally: $\|\mathcal{C}\|_{op} = \epsilon < 1$ is sufficient for all theorems above (Banach fixed-point, geometric convergence on $\{\mathcal{C}^n\}$, Convergent Tower, capacity bound, FDR signatures). Wilson–Kadanoff structural equivalence is a classification conjecture with proof strategy: (i) trail-norm locality factorizes over $\tau_{obs}$-windows with exponentially small cross-terms; (ii) block variables $D_k = \int K(t-t_k)\,d(t)\,dt$ at scale $b$ satisfy $D^{(b)} = f(b, D)$ with the same capacity bound; (iii) an isometry $\phi$ on $\mathcal{T}$ conjugates $\mathcal{C}$ to block-averaging on the slow manifold. Tier 1 is load-bearing for the framework; Tier 2 supports the *RG-like* classification claim and remains open at formal-proof level.
