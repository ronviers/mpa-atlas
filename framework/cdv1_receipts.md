...
### §gFDR / §16 — two-frame gFDR: external-probe (amplitude × field → X) vs self-probe (current × affinity → T) (STAGED CANDIDATE)
**Type:** composition + staged-candidate (NOT promoted to cdv1_compressed; see thin-discipline note).
...
**Thin-discipline note (why staged, not promoted).** This is a *new* claim, and its confirming run is a synthetic co-onset (necessary consistency), not a real cross-substrate instance — the same bar §846 holds $k_{\text{frust}}$ to. Per `mpa-atlas/CLAUDE.md` (thicken on actual force, not anticipated), it stays in receipts until promotion is earned. **Promotion to cdv1_compressed §gFDR / §Thermo-info on either:** (1) a calibrated $\langle\sigma\rangle$/affinity meter closing $\mathrm{SNR}_J\to$ full $T$, *and* a two-frame regime agreement ($T$ vs $X$) on a real substrate where both probes are feasible; or (2) the payoff instance — a real substrate read self-probe-only (no external probe feasible) where $T$ recovers a regime verdict the external frame cannot reach. Either earns the promotion the way a real $k_{\text{frust}}$ instance earns the §8 elevation.

**Owed apparatus.** (a) ~~entropy-production meter~~ **DONE 2026-05-21** — `two_frame_T_meter.py` (binned probability-current $\langle\sigma\rangle$, validated against the exact rotational-OU $2\omega^2/\kappa$); $\mathrm{SNR}_J\to T$ closed on the two sub-regime models. Remaining refinement: a Schnakenberg $J\!\cdot\!A$ cross-check and extension to multiplicative-noise / higher-D substrates. (b) ~~genuine external-frame $X$ measurement~~ **DONE 2026-05-21 (brick 3)** — full $(\chi,\Delta C)$-plane $X$ built + tared on OU vs exact $V_{ext}$, two-frame verdict agreement shown on the OU/SL testbeds. Remaining refinement: exact $V_{ext}=\langle\sigma\rangle$ via the velocity-frame Harada–Sasa integral (brick 3 shows verdict/onset agreement, not numerical identity). (c) The promotion gate proper: $T$ vs $X$ two-frame **agreement on a real substrate** (testbed agreement landed; real cross-substrate instance still owed — same bar §846 holds $k_{\text{frust}}$ to).
...
**Cross-ref.** v9 §Boolean section / v9_receipts §Boolean section (the RM/ANF-ring deformation — the $\triangle_H$ is the chiral generator of circulation *off* the gaugeable Boolean limit; the chimeric sign is the deformation away from a balanced/gaugeable ring); cdv1 §k_frust drain (the affinity drain); §gFDR/§16 two-frame STAGED CANDIDATE (the self-probe affinity is this $\mathcal{A}$); `mpa-conform/docs/banach-substrate-reference.md` companion object. The $k_{\text{frust}}$-primitive elevation (§Open conjectures) stays separately gated; this ignition commitment does not jump that gate.
