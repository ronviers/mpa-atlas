# Naming decision — the framework's honest identity (2026-05-31)

**Status:** recommendation, locked for the §0 first-contact layer (next session). NOT yet planted in canon —
the name is published (Zenodo 10.5281/zenodo.20357550, 4-author byline) and load-bearing across `mpa-*` repos;
the §0 layer lands these in register-clean context, and the deeper-rename question (below) is brought to Ron
deliberately, never slipped in. Decisions made by the integrator on Ron's "no preference" (his judgment
delegated), exercising the discipline's pick-the-obvious-best-and-state-it rule.

## The lineage finding (this is the credential, not the embarrassment)
...
the strongest available honesty signal.

**Etymology of the letters** (inferred; confirm with Ron): MPC = Metastable Propositional **Calculus** →
MPA = Metastable Propositional **Algebra**. The shift Calculus→Algebra *was itself the first deflation* (from
claiming a logic to claiming an operator-algebra deformation of the Boolean ring; engine: "MPA = finite-$D$
deformation of this ring," $\Sigma=\{C,S,K,R,\top,\bot\}$). The old `k` conflict truth-value survived, demoted
from a logic value to `k_frust`, a topological invariant. The three letters *are* the deflation history.
...
## DECISION 2 — the letters / lineage

- **Keep "MPA"** as the citation/technical handle (published, DOI'd, cross-repo — do not orphan it).
- **State the etymology + deflation ONCE, plainly, as the credential:** descends from Metastable Propositional
  Calculus; letters now read ~ Metastable Propositional Algebra; *scaled down by its authors* — the deflation
  worn as honesty. One sentence in §0 or a lineage footnote. Do **not** banner the acronym expansion as the
  identity (see Decision 3).
- **Do NOT lead with the acronym expansion.** "Metastable Propositional Algebra" names only the *structural*
  half (the Boolean-ring spine); the live framework's soul is the **character** projection (driven-dissipative
