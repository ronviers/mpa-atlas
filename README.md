...
- **[`architecture/handoff_*.md`](architecture/)** — pending-work planning documents (v9 architectural-backport, habit-extinction reference driver, protocol tool, operations manual, runs and libraries). Each is self-contained for an agent coming in cold. Completed handoffs are deleted, not preserved (per [CLAUDE.md](CLAUDE.md) §"Handoff completion discipline").
