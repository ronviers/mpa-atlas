...
- **[`architecture/handoff_RFC-S_thin-pass.md`](architecture/handoff_RFC-S_thin-pass.md)** — planning document for RFC-S's thin-pass rewrite (informed v0.2). Names the infinity-machinery available (RG flow, Banach, compactification, sheaves-as-reserve) and the failure modes to watch (pull toward heavyweight, math envy).
- **[`architecture/handoff_*.md`](architecture/)** — pending-work planning documents (v9 architectural-backport, schema files, habit-extinction reference driver, protocol tool). Each is self-contained for an agent coming in cold.
