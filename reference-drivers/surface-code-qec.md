...
**Status:** v0.1 — first reference driver. Hand-built from [mpav1 §FDR signatures](../framework/mpav1_compressed.md) (surface-code identification) and [mpav1 §Substrate-conditional reading rules](../framework/mpav1_compressed.md) (F.1 Markovian sign, F.2 detection events).
...
| **`γ_AB`** (cross-saturation) | Targeted logical-operator error on qubit $A$; syndrome-density change on qubit $B$. | $\gamma_{AB} = \partial(\text{syndrome}_B)/\partial(\text{error}_A) \cdot (G_{0,B} / G_{0,A})$, normalised by local budget. Sign per mpav1 Appendix F.1: negative cooperative, positive conflicting. | Linear in perturbation for $\varepsilon / G_0 < 0.1$. Nonlinear → expand to field kernel (RFC-C Appendix B.1). |
...
| Version | Date | Change |
|---|---|---|
| v0.1 | 2026-05-08 | First reference. Hand-derived from mpav1 §FDR signatures and §Substrate-conditional reading rules. |
